...
## eval（評価）— ワークフローを“テスト”する（任意・少し上級）

「このスキル（ワークフロー）は、ちゃんと意図どおりに動く？」を確かめるための機能です。学校のテストのように、お題を与えて合否を見ます。普段づかいには必須ではありません。

フロー図を開いて、画面上部の **「⚖ Eval」** を押すと開きます。やることは3つ:

1. **テストケースを用意する** — 「こういう入力なら、こうなってほしい」という例を登録します（**AI に自動で作らせる**こともできます）。
2. **評価軸（合格の条件）を決める** — 2種類から選べます:
   - **AI に判定させる** — 「ていねいな言葉づかいか」「必要な項目がそろっているか」などを文章で書くと、AI が合否を判定します。
   - **コードで判定する** — きっちり機械的にチェックしたいとき用（小さな Python。エンジニア向け）。
3. **まとめて実行する** — すべての「テストケース × 評価軸」を実行して、**お題ごとに「合格／不合格」**が一覧で出ます。

ワークフローを直す前と後で結果をくらべれば、「直したことで良くなった／悪くなった」がひと目で分かります。
ワークフローを**実際に動かして**出力を作り、その出力を評価することもできます。その際、削除や送信のような**危険な操作は既定でブロック**され、必要なものだけ承認して実行できます（安全側）。

> 実行すると AI（判定やテストケース生成）が呼ばれるためトークンを消費します。コード評価器の注意点は下記「既知の制約」を参照してください。
