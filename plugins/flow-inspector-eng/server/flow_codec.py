...
def _blank_endpoint_node(node_id: str, ntype: str, title: str, subtitle: str,
    ...
    """Flow start/終了マーカーノードを 1 つ作る (他ノードと同じ shape)。"""
    return {
...
def inject_flow_endpoints(flow: dict) -> dict:
    """フローの先頭に「Flow start」、末尾に「Flow end」マーカーを注入する。
    ...
    start_node = _blank_endpoint_node("flow-start", "trigger", "Flow start", "Inputs / triggers",
                                      "flow.start", cx, min_y - DY)
    end_node = _blank_endpoint_node("flow-end", "parent", "Flow end", "Outputs / notifications",
...
def encode_flow_to_skill_md(flow: dict, body_provider: BodyProvider = passthrough_body_provider) -> str:
    ...
    未対応 (現状の MVP):
      ...
      - ネストされた並列 → TODO
    """
    flow = _strip_flow_endpoints(flow)  # Flow start/終了は本文に書き出さない
...
def encode_flow_to_agent_md(flow: dict) -> str:
    ...
    """
    flow = _strip_flow_endpoints(flow)  # Flow start/完了マーカーは書き出さない
    name = flow.get("name", "unnamed")
...
def encode_flow_to_hook_patch(flow: dict, settings_path: str) -> str:
    ...
    """
    flow = _strip_flow_endpoints(flow)  # Flow start/完了マーカーは hooks に含めない
