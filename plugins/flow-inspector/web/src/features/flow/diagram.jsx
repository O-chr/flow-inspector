...
// ふせんメモコンポーネント
// SVG <foreignObject> 内に HTML を埋め込んで、ドラッグ移動 + テキスト編集を可能に
    ...
    <foreignObject x={note.x} y={note.y} width={note.w} height={note.h} className="sticky-note-fo">
      ...
      >
        <div className="sticky-actions" onMouseDown={(e) => e.stopPropagation()}>
          ...
          <button type="button" className="sticky-action sticky-remove" onClick={(e) => { e.stopPropagation(); onRemove(); }} title="削除">×</button>
        </div>
        {/* 上部のドラッグハンドルバー (drag 専用領域) */}
        <div className="sticky-drag-handle" title="ドラッグで移動" />
        {isEditing ? (
          <textarea
            autoFocus
          ...
          <div className="sticky-text" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); onStartEdit(); }}>
            {note.text || <span className="sticky-placeholder">クリックして編集</span>}
          </div>
        )}
        {/* リサイズハンドル (右下) */}
        <div className="sticky-resize" onMouseDown={onResizeStart} title="ドラッグでサイズ変更" />
      </div>
    </foreignObject>
  );
}
...
export function FlowDiagram({ workflow, selected, onSelect, onAddDraft, onRemoveDraft, drafts = [], onAIDesign, autoFit }) {
  ...
  // ── ふせんメモ機能 ──
  // workflow.id ごとに localStorage 永続化
  ...
  // autoFit: コンテナにフロー全体が収まるよう初期ビューを合わせる（Eval比較ペイン等の小領域用）。
  React.useEffect(() => {
    if (!autoFit) return;
    let raf;
    const doFit = () => {
  ...
  return (
    <div className="diagram-wrap">
      ...
      >
        ...
        <span className="add-note-label">メモ</span>
      </button>
      ...
      {picker && (
        <>
          ...
          <div className="node-picker" style={{ left: picker.x, top: picker.y }}>
            ...
            >
              <span className="node-picker-grip" title="ドラッグで移動">⋮⋮</span>
              <span className="node-picker-title">✨ AIに相談</span>
              <button type="button" className="node-picker-close" onClick={() => setPicker(null)} title="閉じる">×</button>
            </div>
            ...
            <div className="np-divider"><span>または手動で選択</span></div>
            <NodePickerList
              onPick={(item) => {
      ...
      <div className="canvas-help">
        <span>スクロールで拡大</span><span className="dot" /><span>ドラッグで移動</span><span className="dot" /><span>ノードをクリックで詳細</span>
      </div>
    </div>
  );
}
