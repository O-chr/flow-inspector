...
def collect_dashboard_data(projects_root: str = None) -> dict:
    ...
    # ── Build config_stack ──
    layer_defs = [
        ("managed", "MANAGED", "~/.claude/plugins/", "インストール済みプラグイン", "プラグインなし"),
        ("user", "USER GLOBAL", "~/.claude/", "個人設定 · 全プロジェクト共通", "グローバル設定なし"),
        ("user-project", "USER × PROJECT", "~/.claude/projects/", "プロジェクト別ユーザー設定", "プロジェクト固有設定なし"),
        ("project", "PROJECT", "~/projects/*/.claude/", "プロジェクトレベル設定", "プロジェクト設定なし"),
        ("local", "LOCAL", ".claude.local/", "ローカル上書き", "ローカル上書きなし"),
    ]
    config_stack = []
...
def collect_claude_stack(project_path: str, home: Path = None) -> dict:
    ...
    defs = [
        ...
        ("user",         "USER GLOBAL",    home / ".claude" / "CLAUDE.md",
         "全プロジェクト共通",              "グローバル CLAUDE.md なし"),
        ("user-project", "USER × PROJECT", user_project_md,
         "このプロジェクト専用の個人メモ",  "CLAUDE.md なし（MEMORY.md は別管理）"),
        ("project",      "PROJECT",        proj / "CLAUDE.md",
         "git管理・チーム共有",             "プロジェクト CLAUDE.md なし"),
        ("local",        "LOCAL",          proj / "CLAUDE.local.md",
         "gitignore対象（非推奨）",         "ローカル上書きなし — クリックで作成"),
    ]
